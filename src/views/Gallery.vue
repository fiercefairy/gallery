    ...
    row(container, :gutter="12")
      column(v-for="item in galleryData[name]" :xs="12", :md="4", :lg="3")
        router-link(:to="`${name}/${item.objectId}`")
          img(v-if="item.token_info && item.token_info.formats[0].mimeType!=='video/mp4'", :src="`https://cloudflare-ipfs.com/ipfs/${item.token_info.artifactUri.replace('ipfs://','')}`")
          video(v-if="item.token_info && item.token_info.formats[0].mimeType==='video/mp4'", :src="`https://ipfs.io/ipfs/${item.token_info.artifactUri.replace('ipfs://','')}`", loop, autoplay)
